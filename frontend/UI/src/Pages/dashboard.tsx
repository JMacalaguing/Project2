...
const Dashboard: React.FC = () => {
  ...
  const formId = location.state?.formId; // Access the passed formId

  console.log("Form ID in Dashboard:", formId); // Log the formId for debugging
...
MAINTENANCE AND OTHER OPERATING EXPENSES`,
  ...
  const tableComponents = [
    <TableA key="tableA" formId={formId} />, // Pass formId to TableA
    <TableB key="tableB" />, // TableB doesn't need formId
  ];
  ...
  return (
    <div className="main-container min-h-screen w-full">
      ...
      <main className="form-container flex-1 container px-2 py-2 bg-white">
        ...
        <div>{tableComponents[currentIndex]}</div>
      </main>
    </div>
  );
};
